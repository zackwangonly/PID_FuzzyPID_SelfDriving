...
int main()
{
  ...
  */
  ...
  //ofstream fout;
  //fout.open("error.txt");
  //std::cout<<"open the file!"<<endl;
  int cout = 1;
  double Sum_Error = 0;

  

  h.onMessage([&pid_steer, &pid_throttle,&cout,&Sum_Error](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          Sum_Error = Sum_Error + cte*cte;
          std::cout<<"Sum_Error: "<<sqrt(Sum_Error)<<std::endl;
          std::cout<<"Ave_Error: " <<sqrt(Sum_Error)/cout <<std::endl;
          cout ++;
          //control the steer based on cte error
          double w_1 = -0.0267*speed + 1.542;
          double w_2 = -0.025*speed + 1.511;
          double w_3 = -0.0231*speed + 1.499;

          double new_kp = w_1*0.2 + (1-w_1)*0.06;
          double new_ki = w_2*0.001 + (1-w_2)*0.00031;
          double new_kd = w_3*3.5 + (1-w_3)*1.29;
          //double steer_output = 1.0;
          if (speed<=30){
          ...
          //std::cout<<"kp,ki,kd: "<<steer_Kp<<" "<<steer_Ki<<" "<<steer_Kd<<std::endl;
          pid_steer.RenewParameter(new_kp,new_ki,new_kd);
          //pid_steer.Init(steer_Kp, steer_Ki, steer_Kd, steer_output);
